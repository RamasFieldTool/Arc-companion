...
(() => {
  ...
  let guideNorm = {x:.02,y:.03,w:.96,h:.94};
  ...
  async function analyzeSource(source,sw0,sh0,cropNorm,fromCamera){
    ...
    if(!backpack){
      ...
      setStatus('Der Rucksack wurde nicht sicher erkannt. Bitte alle Rucksackreihen vollständig in den grünen Bereich bringen.','error');
      clearWorkPixels(); return;
    }

    const expected=median(backpack.slots.map(s=>(s.w+s.h)/2));
    const quick=detectQuickSlots(imageData,{x:.58,y:.02,w:.41,h:.53},expected);
    const safe=detectSafeSlots(imageData,{x:.58,y:.68,w:.41,h:.30},expected);
  ...
  function detectQuickSlots(imageData,z,expected){
    const pattern=detectAuxPattern(imageData,z,expected,'quick');
    if(pattern.confident) return pattern;
    return detectAuxComponents(imageData,z,expected,'quick');
  }

  function detectSafeSlots(imageData,z,expected){
    const zone=normZone(imageData,z);
    const sub=cropImageData(imageData,zone.x,zone.y,zone.w,zone.h);
    const graySub=toGray(sub),threshold=estimateEdgeThreshold(graySub,sub.width,sub.height);
    const comps=connectedComponents(makeEdgeMap(graySub,sub.width,sub.height,threshold),sub.width,sub.height);
    let cand=dedupeCandidates(comps.map(c=>({...c,x:c.x+zone.x,y:c.y+zone.y})).filter(c=>{
      const ratio=c.w/Math.max(1,c.h),size=(c.w+c.h)/2;
      return ratio>=.62&&ratio<=1.45&&size>=expected*.64&&size<=expected*1.42;
    }));

    const gray=toGray(imageData);
    cand=cand.map(c=>{
      const size=(c.w+c.h)/2;
      const border=borderStrength(gray,imageData.width,imageData.height,c);
      const color=auxColorScore(imageData,c);
      const xPenalty=(c.x-zone.x)/Math.max(1,zone.w);
      const sizePenalty=Math.abs(size-expected)/Math.max(1,expected);
      return {...c,_score:border*.55+color*180-sizePenalty*25-xPenalty*14};
    }).sort((a,b)=>b._score-a._score);

    let anchor=cand[0]||null;
    if(!anchor){
      const pat=detectAuxPattern(imageData,z,expected,'safe');
      return pat;
    }

    const pitch=expected*1.04;
    const slots=[];
    for(let i=0;i<3;i++){
      const box={x:Math.round(anchor.x+i*pitch),y:Math.round(anchor.y),w:Math.round(anchor.w),h:Math.round(anchor.h)};
      const b=borderStrength(gray,imageData.width,imageData.height,box);
      if(i===0||b>threshold*.14){ slots.push({...box,occupied:isOccupiedAuxSlot(imageData,box)}); }
      else break;
    }
    return {slots,confident:slots.length>=1};
  }
  ...
  function detectAuxComponents(imageData,z,expected,type){
    ...
    return {slots:cand.map(c=>({...c,occupied:isOccupiedAuxSlot(imageData,c)})),confident};
  }

  function detectAuxPattern(imageData,z,expected,type){
    ...
    let best=null;
    const step=Math.max(4,Math.round(expected*.08));
    for(const sf of [.82,.90,.98,1.06,1.14]){
      ...
      for(const pf of [1.00,1.05,1.10]){
        ...
        for(let y=zone.y;y<=maxY;y+=step) for(let x=zone.x;x<=maxX;x+=step){
          ...
          if(type==='safe'){
            const box={x:Math.round(x),y:Math.round(y),w:Math.round(size),h:Math.round(size)};
            score+=auxColorScore(imageData,box)*220;
            score-=((x-zone.x)/Math.max(1,zone.w))*18;
          }
          if(ok&&(!best||score>best.score)) best={x,y,size,pitch,score};
        }
      }
    }
    if(!best) return {slots:[],confident:false};
    ...
    const add=(cx,cy,required)=>{
      ...
      if(required||s>Math.max(threshold*.16,baseScore*.30)) slots.push({...box,occupied:isOccupiedAuxSlot(imageData,box)});
    };
    mandatory.forEach(p=>add(p[0],p[1],true)); optional.forEach(p=>add(p[0],p[1],false));
    return {slots,confident:slots.length>=(type==='quick'?4:1)};
  }

  function auxColorScore(imageData,box){
    const {width,height,data}=imageData;
    const m=Math.max(2,Math.round(Math.min(box.w,box.h)*.06));
    const x0=clamp(Math.round(box.x+m),0,width-1),y0=clamp(Math.round(box.y+m),0,height-1);
    const x1=clamp(Math.round(box.x+box.w-m),x0+1,width),y1=clamp(Math.round(box.y+box.h-m),y0+1,height);
    let n=0,sat=0,cornerN=0,cornerSat=0;
    const cornerX=x0+(x1-x0)*.48,cornerY=y0+(y1-y0)*.46;
    for(let y=y0;y<y1;y+=2) for(let x=x0;x<x1;x+=2){
      const p=(y*width+x)*4,r=data[p],g=data[p+1],b=data[p+2],mx=Math.max(r,g,b),mn=Math.min(r,g,b);
      const s=mx?((mx-mn)/mx):0;
      n++; if(mx>75&&s>.32)sat++;
      if(x<=cornerX&&y>=cornerY){cornerN++;if(mx>70&&s>.28)cornerSat++;}
    }
    const all=n?sat/n:0,corner=cornerN?cornerSat/cornerN:0;
    return Math.max(all,corner*1.25);
  }

  function isOccupiedAuxSlot(imageData,box){
    const score=auxColorScore(imageData,box);
    if(score>.055) return true;
    const {width,height,data}=imageData,m=Math.max(3,Math.round(Math.min(box.w,box.h)*.15));
    ...
    for(let y=y0;y<y1;y+=2)for(let x=x0;x<x1;x+=2){const p=(y*width+x)*4,l=data[p]*.299+data[p+1]*.587+data[p+2]*.114;n++;sum+=l;sumSq+=l*l;}
    if(!n)return false;const avg=sum/n,sd=Math.sqrt(Math.max(0,sumSq/n-avg*avg));
    return score>.035&&sd>24;
  }
  ...
  function isOccupiedSlot(imageData,box){
    ...
    for(let y=y0;y<y1;y+=2)for(let x=x0;x<x1;x+=2){const p=(y*width+x)*4,l=data[p]*.299+data[p+1]*.587+data[p+2]*.114;n++;sum+=l;sumSq+=l*l;if(l>130)bright++;}
