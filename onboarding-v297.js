...
const HELP_COPY={
  de:{
    ...
    eventsTitle:'4 // LIVE-EVENTS & ERINNERUNGEN',
    eventsBody:'„Live-Events“ zeigt aktuelle und kommende Kartenbedingungen aus dem offiziellen Embark-Zeitplan. Wähle die Server-Region, in der du spielst; die Uhrzeiten werden automatisch in deiner Gerätezeit angezeigt. „Erinnern“ speichert den Termin im geöffneten Tool und lädt einen Kalendereintrag mit Alarm herunter. Der Kalender ist die zuverlässige Erinnerung, wenn die Web-App geschlossen ist.',
    searchTitle:'5 // ITEM-SUCHE',
    searchBody:'Öffne „Item-Suche“ und gib einen Item- oder Materialnamen ein. Teilbegriffe reichen: Du musst den vollständigen Namen nicht kennen. Die Treffer werden getrennt angezeigt. „Direkte Treffer“ passen mit ihrem eigenen Namen zur Suche. „Durch Recycling erhältlich“ zeigt andere Items, die beim Zerlegen das gesuchte Material liefern; „Ergibt“ nennt den passenden Recycling-Ertrag.',
    questsTitle:'6 // QUESTS',
    ...
    blueprintsTitle:'7 // BAUPLAN-TRACKER',
    ...
    dataTitle:'12 // DATENSTATUS & QUELLEN',
  ...
  en:{
    ...
    supplyBody:'Open “Total Needs” to display the calculated material list. It combines materials from your active station levels and active quests. “Owned” is the amount you entered locally; the app uses it to calculate what is still missing. “Show usage” explains which goals or quests create the requirement. Additional costs such as Coins appear in a separate section.',
    ...
    eventsTitle:'4 // LIVE EVENTS & REMINDERS',
    eventsBody:'“Live Events” shows current and upcoming map conditions from the official Embark schedule. Select the server region you play in; times are automatically displayed in your device timezone. “Remind me” stores the event while the tool is open and downloads a calendar entry with an alarm. The calendar is the reliable reminder when the web app is closed.',
    searchTitle:'5 // ITEM SEARCH',
    searchBody:'Open “Item Search” and enter an item or material name. Partial words are enough; you do not need the complete name. Results are separated deliberately. “Direct matches” are items whose own names match the query. “Available through recycling” shows other items that yield the searched material when recycled; “Yields” identifies the matching output.',
    questsTitle:'6 // QUESTS',
    ...
    blueprintsTitle:'7 // BLUEPRINT TRACKER',
    ...
    dataTitle:'12 // DATA STATUS & SOURCES',
...
function renderHelp(){
  ...
  root.innerHTML=`
    ...
    ${['goals','supply','raid','events','search','quests','blueprints','maps','tips','display','backup','data'].map(section=>`
